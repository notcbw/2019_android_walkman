...
#ifdef CONFIG_SAI_DSD_DISABLE_CPUIDLE
uint    dsd_pm_qos_value = 0; /* 0 value means that device never enter idle mode */
module_param_named(pm_qos_value, dsd_pm_qos_value, uint, S_IWUSR | S_IRUGO);
#endif
...
#ifdef CONFIG_SAI_DSD_DISABLE_CPUIDLE
		if (!test_and_set_bit(0x01, &sai->pm_qos_flag))
			pm_qos_add_request(&sai->pm_qos_req,
				PM_QOS_CPU_DMA_LATENCY, dsd_pm_qos_value);
#endif
	} else {
...
#ifdef CONFIG_SAI_DSD_DISABLE_CPUIDLE
	if (test_and_clear_bit(0x01, &sai->pm_qos_flag))
