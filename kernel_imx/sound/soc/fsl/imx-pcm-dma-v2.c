...
static struct snd_pcm_hardware imx_pcm_hardware = {
	...
	.buffer_bytes_max = 564480, /* DSD256 (256 * 44100), 2ch, 200ms */
...
unsigned long dsd_drain_flag;
unsigned int bus_freq_high_flag;
			      ...
			      struct snd_pcm_hw_params *params)
{
	...
	if (pcm_format == SNDRV_PCM_FORMAT_DSD_U8 ||
		...
		set_bit(0x01, &dsd_drain_flag);

	if (params_rate(params) > 192000) {
		if (!test_and_set_bit(0x01, &bus_freq_high_flag)) {
			printk(KERN_INFO "Start bus_freq_high mode\n");
...
static snd_pcm_uframes_t imx_pcm_pointer(struct snd_pcm_substream *substream)
{
	if (substream->runtime->status->state == SNDRV_PCM_STATE_DRAINING &&
	    test_and_clear_bit(0x01, &dsd_drain_flag))
		imx_cxd3778gf_mute_on_trigger();
...
static int imx_pcm_close(struct snd_pcm_substream *substream)
{
	...
	if (test_and_clear_bit(0x01, &bus_freq_high_flag)) {
		release_bus_freq(BUS_FREQ_HIGH);
		printk(KERN_INFO "End bus_freq_high mode\n");
	}
