...
#define FORCE_RECOVERY_DMIC_CLOCK
...
struct fsl_micfil {
	...
	unsigned int clock_enable_flag;
 ...
 * Read this flag for max 3 times, sleep 100ms
...
/* If BSY flag is remained in timing of clk setting,
 * pdm_clock is set to 22050Hz, and enable DMA,
 * BSY flag is erased and DMIC device can recover.
 */
static int dmic_force_recovery(struct device *dev)
{
	struct fsl_micfil *micfil = dev_get_drvdata(dev);
	int i;
	int ret;
	...
	ret = fsl_set_clock_params(dev, 22050);
	if (ret) {
		dev_err(dev, "failed to 22050 clock rate\n");
		return ret;
	}
...
static int fsl_set_clock_params(struct device *dev, unsigned int rate)
{
	...
	dev_dbg(dev, "fsl_set_clock_params clk_id = 0 rate = %d\n", rate);
				...
				struct snd_soc_dai *dai)
{
	...
	mutex_lock(&micfil->clk_lock);

	ret = fsl_set_clock_params(dev, rate);
	if (ret < 0) {
		dev_err(dev, "Failed to set clock parameters [%d]\n", ret);
		mutex_unlock(&micfil->clk_lock);
		return ret;
	}
				     ...
				     unsigned int freq, int dir)
{
	...
	dev_dbg(dev, "set_dai_sysclk clk_id = %d freq = %d\n", clk_id, freq);
...
#ifdef FORCE_RECOVERY_DMIC_CLOCK
	ret = fsl_micfil_bsy(dev);
	if (ret < 0) {
		dev_err(dev, "Device is busy\n");
		ret = dmic_force_recovery(dev);

		ret = fsl_micfil_bsy(dev);
		if (ret < 0)
			dev_err(dev, "Cannot recover dmic\n");
		else
			dev_dbg(dev, "Recovery done!\n");
	}
#endif
	mutex_unlock(&micfil->clk_lock);
