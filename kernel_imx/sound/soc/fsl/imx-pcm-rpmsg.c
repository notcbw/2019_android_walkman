...
extern void imx_lpa_enable(bool enable);
extern void imx_lpa_dsd_mode_enable(bool enable);
			      ...
			      struct snd_pcm_hw_params *params)
{
	...
	if (rpmsg->send_msg.param.rate > 192000) {
		/* 8fs playback also needs audio pll for dsd playback */
		imx_lpa_dsd_mode_enable(true);
	}
	...
	rpmsg->send_msg.param.buffer_addr = rpmsg_i2s->dai_fmt;
...
static void imx_rpmsg_pcm_dma_complete(void *arg)
{
	 ...
	 *
	 */
	if (i2s_info->suspend[substream->stream] && rpmsg_i2s->force_lpa) {
		int time_msec;
		i2s_info->suspend[substream->stream] = false;
...
int imx_rpmsg_pcm_trigger(struct snd_pcm_substream *substream, int cmd)
{
	...
	case SNDRV_PCM_TRIGGER_START:
		...
		pr_info("%s: SNDRV_PCM_TRIGGER_START -> enable LPA flag.\n", __func__);
		imx_lpa_enable(1);
		...
		wake_unlock(&i2s_info_g->lpa_flag_lock);
		break;
	...
	case SNDRV_PCM_TRIGGER_SUSPEND:
		if (!rpmsg_i2s->force_lpa) {
			...
			i2s_info->suspend[substream->stream] = true;
			del_timer(&i2s_info->stream_timer[substream->stream]);
		}
		break;
	...
	case SNDRV_PCM_TRIGGER_STOP:
		wake_lock_timeout(&i2s_info_g->lpa_flag_lock, msecs_to_jiffies(1000));

		pr_info("%s: SNDRV_PCM_TRIGGER_STOP -> disable LPA flag.\n", __func__);
		imx_lpa_enable(false);
		ret = imx_rpmsg_terminate_all(substream);
		break;
	default:
		return -EINVAL;
...
int imx_rpmsg_pcm_ack(struct snd_pcm_substream *substream)
{
	...
	if (buffer_tail != rpmsg->send_msg.param.buffer_tail) {
		...
		if ((avail - writen_num * runtime->period_size)
					<= runtime->period_size || i2s_info->buffer_full[substream->stream]) {
			spin_lock_irqsave(&i2s_info->wq_lock, flags);
			if (i2s_info->work_write_index != i2s_info->work_read_index) {
				int index = i2s_info->work_write_index;
				memcpy(&i2s_info->work_list[index].msg, rpmsg,
...
int imx_rpmsg_platform_register(struct device *dev)
{
	...
	wake_lock_init(&i2s_info_g->rpmsg_lock, WAKE_LOCK_SUSPEND, "lpa_rpmsg");
	wake_lock_init(&i2s_info_g->lpa_flag_lock, WAKE_LOCK_SUSPEND, "lpa_flag");
			...
			void *priv, u32 src)
{
	...
	if (msg->header.type == I2S_TYPE_C) {
		if (msg->header.cmd == I2S_TX_PERIOD_DONE) {
			wake_lock(&i2s_info_g->rpmsg_lock);
