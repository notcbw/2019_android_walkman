...
{
	int ret;
	struct fsl_rpmsg_i2s *rpmsg_i2s = snd_soc_dai_get_drvdata(cpu_dai);

#ifdef ENABLE_AUDIO_PLL
	clk_prepare_enable(rpmsg_i2s->rpmsg_pll8k);
	clk_prepare_enable(rpmsg_i2s->rpmsg_pll11k);
#endif
...
static int fsl_rpmsg_set_dai_fmt(struct snd_soc_dai *cpu_dai, unsigned int fmt)
{
	...
	rpmsg_i2s->dai_fmt = fmt;

	return 0;
}

static int fsl_rpmsg_shutdown(struct snd_pcm_substream *substream,
		struct snd_soc_dai *cpu_dai)
{
	struct fsl_rpmsg_i2s *rpmsg_i2s = snd_soc_dai_get_drvdata(cpu_dai);

#ifdef ENABLE_AUDIO_PLL
	clk_disable_unprepare(rpmsg_i2s->rpmsg_pll11k);
	clk_disable_unprepare(rpmsg_i2s->rpmsg_pll8k);
#endif
	return 0;
}
...
static int fsl_rpmsg_i2s_probe(struct platform_device *pdev)
{
	...
	/* Setup work queue */
	i2s_info->rpmsg_wq = alloc_ordered_workqueue("rpmsg_i2s", WQ_HIGHPRI | WQ_UNBOUND);
...
#ifdef ENABLE_AUDIO_PLL
	rpmsg_i2s->rpmsg_pll8k = devm_clk_get(&pdev->dev, "pll8k");

	if (IS_ERR(rpmsg_i2s->rpmsg_pll8k)) {
		ret = PTR_ERR(rpmsg_i2s->rpmsg_pll8k);
		dev_err(&pdev->dev, "Cannot get the pll8k clock: %d\n",
			ret);
		return -1;
	}

	rpmsg_i2s->rpmsg_pll11k = devm_clk_get(&pdev->dev, "pll11k");

	if (IS_ERR(rpmsg_i2s->rpmsg_pll11k)) {
		ret = PTR_ERR(rpmsg_i2s->rpmsg_pll11k);
		dev_err(&pdev->dev, "Cannot get the pll11k clock: %d\n", ret);
		return -1;
	}
#endif
