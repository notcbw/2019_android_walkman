...
#ifdef TUNE_TIMING_SYNC_EVENT
	const uint64_t bit_clk =  792000000;
	const uint64_t pixel_clk = 66000000;
#endif
...
#ifdef TUNE_TIMING_SYNC_EVENT
	if (dsim->panel && vmode->hactive == 720 && vmode->vactive == 1280) {
		hfp_wc = vmode->hfront_porch * (bit_clk / 8) / pixel_clk / 2 - 2;
		hbp_wc = vmode->hback_porch * (bit_clk / 8) / pixel_clk - 8;
	}
#else
	/* calculate hfp & hbp word counts */
	hfp_wc = vmode->hfront_porch ; //60 ;//dsim->hpar->hfp_wc;
	hbp_wc =  vmode->hback_porch; //56; // 105; //dsim->hpar->hbp_wc;
#endif
...
struct dsim_pll_pms *sec_mipi_dsim_calc_pmsk(struct sec_mipi_dsim *dsim)
{
	...
	for (m = mrange->min; m <= mrange->max; m++) {
		...
		for (p = pr_new.min; p <= pr_new.max; p++) {
			...
			s_pow_2 = roundup_pow_of_two(
				DIV_ROUND_CLOSEST_ULL(mfin, pfout));
	...
	pll_pms->m = best_m;
	pll_pms->s = best_s;
	pll_pms->m += 3;   /* increase phy bit rate */
				...
				const struct drm_display_mode *mode)
{
	...
	//printk("test pix_clk %d, bit_clk %d\n", pix_clk, bit_clk);
