...
struct ehci_ci_priv {
	...
	struct semaphore	sem_vbus;
#endif /* (defined(CONFIG_USB_ICX_VBUS_SUSPEND_OFF)) */
	struct regulator *reg_vbus;
#if (defined(CONFIG_USB_ICX_VBUS_SUSPEND_OFF))
#define	ENABLE_VBUS_EARLY_ON	(0x00000001)
#define	ENABLE_VBUS_ROOT_HUB	(0x00000002)
	ulong		enable_vbus;
	bool		vbus_on;
#endif /* (defined(CONFIG_USB_ICX_VBUS_SUSPEND_OFF)) */
};
...
/* Switch VBUS according to enable request bits.
 * @hcd Host controller device context
 *
 * @pre Lock priv->sem_vbus
 */
static int ci_ehci_reg_vbus_switch(struct usb_hcd *hcd)
{	struct ehci_ci_priv	*priv;
	int	ret = 0;
	bool	turn_to = false /* off */;

	priv = (struct ehci_ci_priv *)(hcd_to_ehci(hcd)->priv);
	...
	if (ret != 0) {
		dev_err(hcd->self.controller,
			...
			turn_to ? "enable" : "disable", ret);
	}
	return ret;
}

static int ci_ehci_reg_vbus_on(struct usb_hcd *hcd, ulong mask)
{	struct ehci_ci_priv	*priv;
	int	result = 0;
	...
	down(&(priv->sem_vbus));
	priv->enable_vbus |= mask;
	result = ci_ehci_reg_vbus_switch(hcd);
	up(&(priv->sem_vbus));

	return	result;
}

static int ci_ehci_reg_vbus_off(struct usb_hcd *hcd, ulong mask)
{	struct ehci_ci_priv	*priv;
	int	result = 0;
...
static int ci_ehci_reg_vbus_suspend(struct usb_hcd *hcd)
{	struct ehci_ci_priv	*priv;
	int			ret = 0;
	...
	if ((priv->enable_vbus) && (priv->vbus_on)) {
		/* Someone On VBUS */
		/* Off VBUS during suspend. */
		ret = regulator_disable(priv->reg_vbus);
		if (ret) {
			/* Can not disable regulator. */
			dev_err(hcd->self.controller,
				"Can not disable regulator. ret=%d\n",
				ret
			);
		}
		priv->vbus_on = false;
...
static int ci_ehci_reg_vbus_resume(struct usb_hcd *hcd)
{	struct ehci_ci_priv	*priv;
	int			ret = 0;
	...
	if ((priv->enable_vbus) && (!(priv->vbus_on))) {
		/* Someone On VBUS */
		/* Note: vbus_on must be false. */
		/* On VBUS. */
		ret = regulator_enable(priv->reg_vbus);
		if (ret) {
			/* Can not enable regulator. */
			dev_err(hcd->self.controller,
				"Can not enable regulator. ret=%d\n",
				ret
			);
		}
		priv->vbus_on = true;
...
#else /* (!defined(CONFIG_USB_ICX_VBUS_SUSPEND_OFF)) */
		if (enable) {
			/* On Root hub VBUS. */
			ret = ci_ehci_reg_vbus_on(hcd,
				ENABLE_VBUS_ROOT_HUB
			);
		} else {
			/* Off Root hub VBUS. */
			ret = ci_ehci_reg_vbus_off(hcd,
				ENABLE_VBUS_ROOT_HUB
			);
		}
		if (ret)
...
static int host_start(struct ci_hdrc *ci)
{
	...
	priv->reg_vbus = NULL;
#else /* (!defined(CONFIG_USB_ICX_VBUS_SUSPEND_OFF)) */
	sema_init(&(priv->sem_vbus), 1);
	/* Use regulator context at suspend and resume. */
...
#else /* (!defined(CONFIG_USB_ICX_VBUS_SUSPEND_OFF)) */
			ret = ci_ehci_reg_vbus_on(hcd, ENABLE_VBUS_EARLY_ON);
			if (ret) {
				/* regulator may not be configured. */
				goto put_hcd;
			}
		} else {
			/* Do nothing . */
			/* We already took regulator context
...
#else /* (!defined(CONFIG_USB_ICX_VBUS_SUSPEND_OFF)) */
		ci_ehci_reg_vbus_off(hcd, ENABLE_VBUS_EARLY_ON);
#endif /* (!defined(CONFIG_USB_ICX_VBUS_SUSPEND_OFF)) */
put_hcd:
	usb_put_hcd(hcd);
	...
	if (hcd) {
		...
		if (ci->platdata->reg_vbus && !ci_otg_is_fsm_mode(ci) &&
			(ci->platdata->flags & CI_HDRC_TURN_VBUS_EARLY_ON))
				...
				regulator_disable(ci->platdata->reg_vbus);
#else /* (!defined(CONFIG_USB_ICX_VBUS_SUSPEND_OFF)) */
				ci_ehci_reg_vbus_off(hcd,
					ENABLE_VBUS_EARLY_ON);
#endif /* (!defined(CONFIG_USB_ICX_VBUS_SUSPEND_OFF)) */
...
#if (defined(CONFIG_USB_ICX_VBUS_SUSPEND_OFF))
	/* Note: Save register context before
	 * VBUS state changing. We expect register
	 * context may be more stable.
	 */
	if (ci->platdata->flags & CI_HDRC_VBUS_SUSPEND_OFF) {
		/* Off VBUS during suspend. */
		ci_ehci_reg_vbus_suspend(ci->hcd);
	}
#endif /* (defined(CONFIG_USB_ICX_VBUS_SUSPEND_OFF)) */
}
...
#if (defined(CONFIG_USB_ICX_VBUS_SUSPEND_OFF))
	/* Note: Restore register context before
	 * VBUS state changing. We expect register
	 * context may be more stable.
	 */
	if (ci->platdata->flags & CI_HDRC_VBUS_SUSPEND_OFF) {
		/* Off VBUS during suspend. */
		ci_ehci_reg_vbus_resume(ci->hcd);
	}
#endif /* (defined(CONFIG_USB_ICX_VBUS_SUSPEND_OFF)) */
}
