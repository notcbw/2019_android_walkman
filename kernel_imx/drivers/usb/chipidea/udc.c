...
__acquires(ci->lock)
{
	...
	spin_unlock(&ci->lock);
	if ((ci->gadget.speed != USB_SPEED_UNKNOWN) && ci->driver) {
		usb_gadget_udc_reset(&ci->gadget, ci->driver);
		/* Set VBUS current 2.5mA (2mA).
		 * Back to Default State, the VBUS current
		 * is limited to 2.5mA.
		 */
		usb_gadget_vbus_draw(&ci->gadget, 2);
	}
...
static irqreturn_t udc_irq(struct ci_hdrc *ci)
{
	...
	if (intr) {
		...
		if ((USBi_SLI & intr) && !(ci->suspended)) {
			...
			if (ci->gadget.speed != USB_SPEED_UNKNOWN &&
			    ci->driver && ci->driver->suspend) {
				spin_unlock(&ci->lock);
				ci->driver->suspend(&ci->gadget);
				spin_lock(&ci->lock);
			}
			pr_debug("set USB_STATE_SUSPENDED by USBi_SLI\n");
			usb_gadget_set_state(&ci->gadget,
					USB_STATE_SUSPENDED);
		}
		retval = IRQ_HANDLED;
...
static void udc_suspend(struct ci_hdrc *ci)
{
	...
	if (ci->driver && ci->vbus_active) {
		if (ci->gadget.state == USB_STATE_SUSPENDED)
			pr_debug("already USB_STATE_SUSPENDED staus\n");
