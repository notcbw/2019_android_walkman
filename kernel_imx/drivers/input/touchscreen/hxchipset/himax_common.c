...
#ifdef HX_ZERO_FLASH
static void himax_request_firmware_async_cb(const struct firmware *fw,
					    void *context)
{
	struct himax_ts_data *ts = context;

	D("%s, Entering\n", __func__);
	ts->fw_entry = fw;
	if (ts->fw_entry == NULL)
		E("%s, Failed to get firmware entry\n", __func__);
}
#endif
...
FW_force_upgrade:
	...
	request_firmware_nowait(THIS_MODULE, FW_ACTION_HOTPLUG,
				i_CTPM_firmware_name, ts->dev, GFP_KERNEL,
				ts, himax_request_firmware_async_cb);
