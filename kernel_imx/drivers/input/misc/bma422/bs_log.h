...
#define LOG_LEVEL LOG_LEVEL_I
	...
	{\
		if (debug_log_level >= LOG_LEVEL_DF)\
			printk(KERN_INFO "[DF]" MODULE_TAG \
				"<%s><%d>" fmt "\n", __func__, __LINE__, ##args);\
	} while (0)
	...
	{\
		if (debug_log_level >= LOG_LEVEL_DA)\
			printk(KERN_INFO "[DA]" MODULE_TAG \
				"<%s><%d>" fmt "\n", __func__, __LINE__, ##args);\
	} while (0)
	...
	/*! print debug fw download message */
	#define PDEBUG_FWDL(fmt, args...) printk(KERN_INFO "[DF]" MODULE_TAG \
		"<%s><%d>" fmt "\n", __func__, __LINE__, ##args)
	#else
	/*! invalid message */
	...
	/*! print debug data log message */
	#define PDEBUG_DLOG(fmt, args...) printk(KERN_INFO "[DA]" MODULE_TAG \
		"<%s><%d>" fmt "\n", __func__, __LINE__, ##args)
	#else
	/*! invalid message */
