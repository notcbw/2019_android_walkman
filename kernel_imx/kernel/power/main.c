...
#define LPA_DSD_MODE_MASK               0x000000F0
#define LPA_DSD_MODE_CLEAR_MASK         0xFFFFFF0F
#define LPA_DSD_MODE_ENABLED            0x000000D0
...
void imx_lpa_dsd_mode_enable(bool enable)
{
	uint32_t val;

	/* Retrieve SRC GPR9 register content */
	val = readl(src_gpr9_reg);
	/* Clear LPA DSD mode Flag */
	val &= LPA_DSD_MODE_CLEAR_MASK;
	/* Update content */
	if (enable) {
		val |= LPA_DSD_MODE_ENABLED;
		pr_info("LPA: DSD/PCM8fs mode enabled\n");
	} else {
		pr_info("LPA: DSD/PCM8fs mode disabled\n");
	}
	writel(val, src_gpr9_reg);
}

bool imx_lpa_dsd_mode_is_enabled(void)
{
	uint32_t val;

	/* Retrieve SRC GPR9 register content */
	val = readl(src_gpr9_reg) & LPA_DSD_MODE_MASK;

	return (val == LPA_DSD_MODE_ENABLED);
}

static ssize_t lpa_dsd_mode_show(struct kobject *kobj,
				struct kobj_attribute *attr, char *buf)
{
	return sprintf(buf, "%u\n", imx_lpa_dsd_mode_is_enabled());
}

static ssize_t lpa_dsd_mode_store(struct kobject *kobj,
				struct kobj_attribute *attr,
				const char *buf, size_t n)
{
	unsigned long val;
	...
	imx_lpa_dsd_mode_enable(val);

	return n;
}

power_attr(lpa_dsd_mode);
